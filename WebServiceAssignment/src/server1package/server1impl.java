...
public class server1impl implements  server1interface{
	...
	public String bookroom(String campusName,String rno,String date,String timeslot,String UID)
	{
			...
			{	
				...
				fwriter(rno+"	"+date+"	"+timeslot+" 	"+bookingid+"booking successful in DVL","DVLServer");
	...
	public String localcount(String date)
	{
	  ...
	  public String  cancelBooking (String bookingID, String userid)
	  {
						...
						{	
							...
							while(idb.hasNext())
							{
				  					...
				  					else
								{
									...
									try 
									{	
										fwriter(s+"	"+s1+"	"+s2+" 	booking cancellation unsuccessful in DVL",userid);
										fwriter(s+"	"+s1+"	"+s2+" 	booking cancellation unsuccessful in DVL","DVLServer");
									} 
									catch (IOException e) 
									{
										e.printStackTrace();
			...
			try 
			{	
				fwriter(bookingid+"booking cancellation unsuccessful in DVL",userid);
				fwriter(bookingid+"booking cancellation unsuccessful in DVL","DVLServer");
			} 
			catch (IOException e) 
			{
				e.printStackTrace();
	  				...
	  				while(it2.hasNext())
	  				{
						...
						while(setn1.hasNext())
						{
	  					...
	  					else
	  					{
	  						...
	  						try 
							{	
								fwriter(s+"	"+s1+"	"+s2+" 	"+bookingID+"booking cancellation unsuccessful in DVL",userid);
								fwriter(s+"	"+s1+"	"+s2+" 	"+bookingID+"booking cancellation unsuccessful in DVL","DVLServer");
							} 
							catch (IOException e) 
							{
								e.printStackTrace();
					...
					if(ret.equals("its already booked"))
			           {
			        	   try 
			          		{	
			          			fwriter(new_room_no+"	"+new_date+"	"+new_time_slot+" 	"+ret+" change reservation  in DVL",studentid);
			          			fwriter(new_room_no+"	"+new_date+"	"+new_time_slot+" 	"+ret+" change reservation  in DVL","DVLServer");
			          		} 
			          		catch (IOException e) 
			          		{
			          			e.printStackTrace();
			           ...
			           else
			           {
			        	   ...
			        	   try 
			          		{	
			          			fwriter(new_room_no+"	"+new_date+"	"+new_time_slot+" 	"+ret+" change reservation successful in DVL",studentid);
			          			fwriter(new_room_no+"	"+new_date+"	"+new_time_slot+" 	"+ret+" change reservation successful in DVL","DVLServer");
			          		} 
			          		catch (IOException e) 
			          		{
			          			e.printStackTrace();
